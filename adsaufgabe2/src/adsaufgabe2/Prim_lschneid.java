...
public class Prim_lschneid implements IPrim {
    ...
    public boolean computeMSB(IGraph graph, int sourceVertex) {
    	...
    	while(knotenVorhanden(graph)) {
    		...
    		for(int i = 0; i <= graph.getNumberVertices() - 1; i++) {
    			if(graph.containsEdge(i, naechster) && ( abstand[i] == -1 || abstand[i] > graph.getWeight(naechster, i)) && istFertig[i] == false) {
    ...
    private int naechsterKnoten(IGraph graph) {
    	int minimumWert = -1;
    	int naechster = -1;
    	for(int i = graph.getNumberVertices() - 1; i >= 0; i--) {
    		boolean valueIsSmaller = abstand[i] != -1 && abstand[i] <= minimumWert;
    		if(( minimumWert == -1 || valueIsSmaller ) && istFertig[i] == false) {
